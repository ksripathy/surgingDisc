...
datadir.file = 'FINAL*.*';
datafiles = dir(fullfile(datadir.main,datadir.file));
...
for K=1
    ...
    datadir.file = datafiles(K).name;  
    DATA=load(fullfile(datafiles.folder,datadir.file));
...
% x_sta=[-0.45:0.05:-0.1];
x1=[-0.1:0.001:0.2];  % % % % % tests show that the load is very sensitive to
% the right boundary.
...
xline= [zeros(size(y1))+min(x1) x1 zeros(size(y1))+max(x1) fliplr(x1)];  
yline= [ y1 zeros(size(x1))+max(y1)  fliplr(y1) zeros(size(x1))+min(y1)]; 
zline = yline.*0;
