...
class planarPIVField():
    ...
    def __init__(self, meanFilePath, logFilePath, discXc, discYc):
        ...
        for i,tecData in enumerate(self.framesTecData):
            
            setattr(self,f"frame{i}",pivFrames2D(self.framesCols[i],self.framesRows[i],tecData,discXc,discYc))
        
        self.frame0 = self.frame0.flipGrid(flipAxis=None)
        self.ndimFrame0 = self.frame0.ndimData(self.rhoInf, self.VInf)
        
        self.frame1 = self.frame1.flipGrid(flipAxis=None)
        self.ndimFrame1 = self.frame1.ndimData(self.rhoInf, self.VInf)
        
        self.field = self.combineFrames(self.frame0,self.frame1,True)
        self.ndimField = self.combineFrames(self.ndimFrame0,self.ndimFrame1,True)
        
        self.ndimField.genInterpolator()
        self.ndimFieldFilled = self.fillDiscMask(self.ndimField)
        
        xcLine = np.tile(self.ndimField.discXc,21)
        xuLine = np.tile(self.ndimField.discXc - 0.1*self.ndimField.discDia,21)
        yLine = np.linspace(self.ndimField.discYc - 0.5*self.ndimField.discDia,self.ndimField.discYc + 0.5*self.ndimField.discDia,21)
        
        self.discCentreLine = np.column_stack((xcLine,yLine))
        self.discUstreamLine = np.column_stack((xuLine,yLine))
        
        
        
        # self.indVxuRaw, self.indVyuRaw, self.indVuRaw = self.getIndVelRaw(self.discUstreamLine)
        # self.indVxcRaw, self.indVycRaw, self.indVcRaw = self.getIndVelRaw(self.discCentreLine)
        # self.indVxcIntp = self.getDiscVel(self.ndimField)
        
        # rbfMask = self.getRBFMask(self.ndimField)
        
        
        # ustrmRBFMask, dstrmRBFMask, sumRBFMask, diffRBFMask = self.getRBFMask3(self.ndimField,ustrOffset=0.1,ustrmPolygonWidth=1,dstrOffset=0.15,dstrmPolygonWidth=1)
        # self.ndimField.genRBFInterpolator(ustrmRBFMask,skipDims=3)
        
        # self.ndimField.genRBFInterpolator2(ustrmRBFMask,dstrmRBFMask,skipDims=4,rbfKernel="linear")
        
        # self.indVxuRaw, self.indVyuRaw, self.indVuRaw = self.getIndVelRaw(self.discUstreamLine)
        # self.indVxuIntp, self.indVyuIntp, self.indVuIntp = self.getIndVelIntp(self.discUstreamLine)
        
        # self.indVxcRaw, self.indVycRaw, self.indVcRaw = self.getIndVelRaw(self.discCentreLine)
        # self.indVxcIntp, self.indVycIntp, self.indVcIntp = self.getIndVelIntp(self.discCentreLine)
